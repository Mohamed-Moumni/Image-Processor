...
from ..serializers.transformation_serializer import FilterTransformationSerializer, ChangeFormatTransformationSerializer, CompressTransformationSerializer, FlipTransformationSerializer, RotateTransformationSerializer, CropTransformationSerializer, ResizeTransformationSerializer
...
class TransformationFilterView(APIView):
    def post(self, request, id:int):
        serializer = FilterTransformationSerializer(data=request.data)
        if serializer.is_valid():
            try:
                trans_service = TransformationService()
                img_serv = ImageService()
                image = img_serv.get(id)
                transformed_image = trans_service.apply_filter(image, request.user.id, **serializer.validated_data)
                return Response(data=transformed_image, status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"error": str(e)}, status=status.HTTP_404_NOT_FOUND)
        else:
